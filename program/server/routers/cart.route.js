...
router.post(
  "/getCartItems",
  [check("items", "0 предметов в заказе").exists()],
  async (req, res) => {
    try {
...
router.post(
  "/saveOrder",
  [
    check("startDate", "Дата начала не может быть пустым").exists(),
    check("endDate", "Дата конца не может быть пустым").exists(),
    check("addres", "Адрес не может быть пустым").exists(),
  ],
  auth,
  async (req, res) => {
    try {
